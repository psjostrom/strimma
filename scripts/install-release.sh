...
echo "==> Installing release APK to $selected_serial..."
adb -s "$selected_serial" install -r -d "$repo_root/app/build/outputs/apk/release/app-release.apk"

echo "==> Launching release app..."
adb -s "$selected_serial" shell am start -n com.psjostrom.strimma/com.psjostrom.strimma.ui.MainActivity
