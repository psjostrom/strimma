...
Libre 3 → CamAPS FX (BLE) → notification → GlucoseNotificationListener
  → StrimmaService → Room DB → SpringaPusher → Springa API
...
- `network/` — SpringaClient + SpringaPusher (Ktor, Nightscout-compatible JSON)
...
- **Springa push:** Immediate on new reading. Retry with linear backoff. Unpushed readings marked `pushed=0` in Room.
...
- Runs side-by-side with xDrip+ during validation (separate Springa tables)
